/**
 * @file scenarios/turn_oncoming_first.h
 * @brief Turn Oncoming First scenario — ego yields, oncoming crosses first.
 *
 * 3D state: [s_ego, v_ego, s_oncoming]
 * 1D input: [T_ego] (torque)
 *
 * Matches turn_oncoming_first_dynamics.cl exactly.
 * v_oncoming is a SENSED parameter (not a state), updated at runtime.
 *
 * Key differences from turn_ego_first:
 *   - Safety: ego_zone > oncoming_zone is unsafe (ego must NOT lead)
 *   - Self-loop: when safe AND s_oncoming >= 10 (oncoming has cleared)
 *   - Oncoming velocity: V0_MIN = 8.0 (worst-case slowest)
 *   - Parameter corners: yield-optimized (different from ego-first)
 *   - Synthesis macro: V0_MIN (not V0_MAX)
 */
...
class TurnOncFirstDynamics : public DynamicsModel {
public:
    // Vehicle parameters (from turn_oncoming_first_dynamics.cl constants)
    ...
    double v_oncoming = 8.0;  // default: worst-case V0_MIN

    // -----------------------------------------------------------------------
    // Metadata
    // -----------------------------------------------------------------------
    std::string name() const override { return "turn_oncoming_first"; }
    ...
    void set_param(const std::string& name, double value) override {
        if (name == "v_oncoming" || name == "V0_MIN") v_oncoming = value;
    ...
    bool is_safe_condition(double s_ego, double s_oncoming) const {
        ...
        // Oncoming First: unsafe if ego cuts in front (ez > oz) or both in zone 2
        return !((ez > oz) || (ez == 2 && oz == 2));
    }

    // -----------------------------------------------------------------------
    // Vehicle acceleration (matches .cl exactly — yield worst-case corners)
    //
    // Note: parameter corners differ from turn_ego_first because the
    // yield strategy needs MINIMUM braking capability (worst case for ego).
    // -----------------------------------------------------------------------
    double compute_vehicle_accel(double v, double T) const {
        double R_w = (T > 0.0) ? R_W_MIN : R_W_MAX;
        double M   = ((T / R_w - ALPHA_MIN) > 0.0) ? M_MIN : M_MAX;
        double a   = (1.0 / M) * (T / R_w - ALPHA_MIN);
    ...
    void ode(const double* x, const double* u, double* dx) const override {
        ...
        // Self-loop: oncoming has cleared intersection while safe
        if (is_safe_condition(s_ego, s_onc) && s_onc >= 10.0) {
            dx[0] = dx[1] = dx[2] = 0.0;
            return;
        }
    ...
    CostParams default_cost_params() const override {
        return {
            ...
            {"w_terminal_v", 10.0},
            {"w_jerk",       0.001},
            {"goal",         -10.0},    // approach intersection boundary
            {"unsafe_goal",  -40.0}     // retreat when unsafe
        };
    }
                                 ...
                                 const CostParams& p) const override
    {
        ...
        const double w_tv   = cparam(p, "w_terminal_v", 10.0);
        const double w_jerk = cparam(p, "w_jerk",       0.001);
        const double goal   = cparam(p, "goal",         -10.0);
        const double ugol   = cparam(p, "unsafe_goal",  -40.0);
        const int    nxd    = nx();
        const int    nud    = nu();
                   ...
                   const Eigen::MatrixXd& u, const double& /*slack*/) -> double
        {
            ...
            // Terminal velocity penalty (unsafe only — encourage braking to yield)
            if (!safe0 && w_tv > 0.0) {
                double vH = x(H, 1);
                cost += w_tv * vH * vH;
            }
