#!/usr/bin/env python3
"""Exhaustive correctness oracle for the four production schedules.
...
def run_cdc(widths: Sequence[int], successor: Successor) -> Tuple[Set[Point], List[Set[Point]]]:
    universe = grid(widths)
    current = set(universe)
    trace = [set(current)]
    while current:
        basis = maximal(current)
        unsafe = next((point for point in basis if not controlled(point, current, successor)), None)
        if unsafe is None:
            break
        # Literal immediate CDC mutation: consume exactly the first unsafe
        # generator, expose its lower neighbours via Bas(K \ {unsafe}), restart.
        current.remove(unsafe)
...
def assert_four_methods(widths: Sequence[int], successor: Successor) -> None:
    cdc, _ = run_cdc(widths, successor)
    ...
    hashes = {
        ...
        for result in (cdc, auto_scan, auto_threshold, proposed)
    }
    assert len(hashes) == 1
...
def check_exhaustive_2x2() -> int:
    ...
    for successor in enumerate_monotone_successors((2, 2)):
        assert_four_methods((2, 2), successor)
        count += 1
    assert count > 0
    return count
...
def check_known_examples() -> None:
    ...
    cdc, cdc_trace = run_cdc(widths, successor)
    auto, auto_trace = run_automatica(widths, successor)
    assert cdc == auto == set()
    assert [len(value) for value in cdc_trace] == [4, 3, 2, 1, 0]
    ...
    assert_four_methods((4, 4), successor_4)


def check_config_contract() -> None:
    production = {"cdc", "automatica_scan", "automatica_threshold", "threshold"}
    ...
    runner_path = Path(__file__).resolve().parents[1] / "tools" / "run_four_method_benchmark.py"
    spec = importlib.util.spec_from_file_location("four_method_runner", runner_path)
...
def main() -> int:
    ...
    print(f"four-method oracle passed ({count} monotone 2x2 systems)")
    return 0
